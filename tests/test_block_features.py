...
def test_derive_adds_feature_columns():
    extractor = BlockFeatureExtractor()
    df = extractor.derive(_ROWS)
...
def test_derive_base_fee_gwei_conversion():
    extractor = BlockFeatureExtractor()
    df = extractor.derive(_ROWS)
    assert df["base_fee_gwei"].iloc[0] == pytest.approx(10.0, abs=1e-6)


def test_derive_trend_zero_within_lookback():
    extractor = BlockFeatureExtractor()
    df = extractor.derive(_ROWS)
...
def test_derive_trend_nonzero_after_lookback():
    extractor = BlockFeatureExtractor()
    df = extractor.derive(_ROWS)
    assert df["base_fee_trend"].iloc[BlockFeatureExtractor.TREND_LOOKBACK] != 0.0


def test_fetch_latest_raises_without_client():
    extractor = BlockFeatureExtractor()
    with pytest.raises(RuntimeError, match="No Etherscan client"):
        extractor.fetch_latest()


def test_fetch_latest_calls_client_and_returns_dataframe():
    client = MagicMock()
    ...
    df = extractor.fetch_latest()
    assert isinstance(df, pd.DataFrame)
    assert len(df) == BlockFeatureExtractor.TREND_LOOKBACK + 1
    assert "base_fee_gwei" in df.columns


def test_fetch_latest_raises_when_no_blocks_returned():
    client = MagicMock()
